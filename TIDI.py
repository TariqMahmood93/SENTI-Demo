...
<div id="senti-splash">
    ...
    <div class="splash-msg">A smart data imputation tool powered by AI.<br>Please wait while the tool is loading&hellip;</div>
...
def _inject_master_css():
    ...
    :root {
        ...
        --panel2:   #C2D6E8;
        --rim:      rgba(0,0,0,0.25);
        --rim2:     rgba(0,0,0,0.40);
        --teal:     #0070C0;
        --teal2:    #005A9E;
        --teal3:    rgba(0,112,192,0.20);
        --teal4:    rgba(0,112,192,0.10);
        --amber:    #B45309;
        --amber2:   rgba(180,83,9,0.15);
        --rose:     #BE123C;
        --rose2:    rgba(190,18,60,0.12);
        --lilac:    #6D28D9;
        --lilac2:   rgba(109,40,217,0.12);
        --sky:      #0369A1;
        --sky2:     rgba(3,105,161,0.15);
        --green:    #047857;
        --text:     #000000;
        --text2:    #1a1a1a;
        --text3:    #333333;
    ...
    html, body, [data-testid="stAppViewContainer"] {
        background: linear-gradient(180deg, #f0f4f8 0%, #dde8f2 100%) !important;
    ...
    [data-testid="stAppViewContainer"]::before {
        ...
        background-image:
            linear-gradient(rgba(0,112,192,0.07) 1px, transparent 1px),
            linear-gradient(90deg, rgba(0,112,192,0.07) 1px, transparent 1px);
    ...
    [data-testid="stSidebar"] [data-testid="stRadio"] label:has(input:checked) {
        background: rgba(0,112,192,0.20) !important;
    ...
    [data-testid="stRadio"] label:has(input:checked) {
        border-color: #0070C0 !important;
        color: #000000 !important;
        background: rgba(0,112,192,0.22) !important;
    }
    ...
    * { scrollbar-width: thin; scrollbar-color: var(--panel2) var(--ink); }
    ...
    .SENTI-delta-change { font-size: 1.198rem; color: var(--teal2); font-weight: 600; }

    /* Pipeline */
    .SENTI-pipeline { display: flex; gap: 0; margin-bottom: 18px; }
    .SENTI-pipeline-step {
        flex: 1; padding: 8px 10px; text-align: center;
        background: rgba(240,244,248,0.8); border: 1px solid var(--rim);
        font-family: var(--mono); font-size: 0.998rem;
    ...
    /* Column type badges */
    .col-N { display:inline-block; font-family:var(--mono); font-size: 0.86rem; padding:1px 4px; border-radius:3px; margin-right:4px; background:var(--sky2); color:var(--sky); border:1px solid rgba(56,189,248,0.2); }
    .col-C { display:inline-block; font-family:var(--mono); font-size: 0.86rem; padding:1px 4px; border-radius:3px; margin-right:4px; background:var(--amber2); color:var(--amber); border:1px solid rgba(255,184,0,0.2); }
        ...
        if strategy == "SENTI":
            ...
            if imputed is None:
                ...
                with st.expander("Error log", expanded=True):
                    st.markdown(
                        f'<pre style="background:#0f172a;color:#FCA5A5;font-size:1.136rem;'
...
def page_null_injection():
    ...
    engine = st.radio(
        "Injection engine",
        ["🔧  SENTI built-in", "🔬  pyampute (multivariate)"],
        horizontal=True,
...
def _custom_imputer_panel():
    ...
    .repo-card-desc { font-size:1.136rem; color:#94A3B8 !important; line-height:1.5; }
    </style>
    """, unsafe_allow_html=True)
                    ...
                    if matched:
                        for title, explanation, fix_cmds in matched:
                            diag_html = (
                                ...
                                + explanation +
                                '</div>'
                                '<div style="font-size:1.136rem;font-weight:600;color:#94A3B8;'
   ...
   # _section("Configuration", badge="SETUP")
    ...
    with m3:
        if ss.strategy == "SENTI":
            ...
            st.markdown(
                '<div style="font-size:1.198rem;color:var(--text2);line-height:1.55;padding-top:3px">'
            ...
            st.markdown(f'<div style="font-size:1.229rem; color:var(--text2); padding-top:0.4rem; line-height:1.6;">{_desc.get(ss.strategy, "")}</div>',
                        unsafe_allow_html=True)
    ...
    if ss.working_df is not None:
        ...
        if ss._impute_col_mode == "manual":
            ...
            st.markdown(
                f'<div style="font-size:1.198rem;color:var(--text2);padding:4px 0;">' 
    ...
    if ss.working_df is not None:
        if ss.mode == "Static Imputation":
            ...
            with hint_col:
                st.markdown(
                    '<div style="padding-top:0.5rem; font-size:1.106rem; color:var(--text3);">'

...
def _inject_master_css():
    ...
    :root {
        ...
        --teal:     #00F5D4;
        --teal2:    #00C4AA;
        --teal3:    rgba(0,245,212,0.10);
        --teal4:    rgba(0,245,212,0.06);
        --amber:    #FFB800;
        --amber2:   rgba(255,184,0,0.10);
        --rose:     #FF4D6D;
        --rose2:    rgba(255,77,109,0.10);
        --lilac:    #A78BFA;
        --lilac2:   rgba(167,139,250,0.10);
        --sky:      #38BDF8;
        --sky2:     rgba(56,189,248,0.10);
        --green:    #34D399;
        --text:     #E8EBF4;
        --text2:    #8A90AA;
        --text3:    #454B62;
    ...
    /* ── Global white text for ALL widget labels ────────── */
    ...
    [data-testid="stSidebar"]::after {
        ...
        background: linear-gradient(to bottom, transparent, rgba(0,245,212,0.2) 40%, rgba(0,245,212,0.2) 60%, transparent);
    }
    [data-testid="stSidebar"] * {
        color: var(--text) !important;
        font-family: var(--sans) !important;
    ...
    .stButton > button, .stDownloadButton > button {
        ...
        box-shadow: 0 0 20px rgba(0,245,212,0.18) !important;
        transition: all 0.15s ease !important;
    }
    .stButton > button:hover, .stDownloadButton > button:hover {
        background: #18FFDE !important;
        box-shadow: 0 0 30px rgba(0,245,212,0.35) !important;
    ...
    div[data-baseweb="select"] > div:hover { border-color: var(--rim2) !important; }
    div[data-baseweb="popover"] { background: var(--panel2) !important; border: 1px solid var(--rim2) !important; }
    [data-baseweb="list-item"] { background: var(--panel2) !important; color: var(--text) !important; }
    [data-baseweb="list-item"]:hover { background: var(--panel) !important; }
    ...
    [data-testid="stFileUploader"] section {
        background: var(--panel) !important;
        border: 2px dashed rgba(0,245,212,0.2) !important;
    ...
    .stDataFrame iframe { background: var(--panel) !important; }
    ...
    [data-testid="stMetricLabel"] {
        color: var(--text3) !important; font-size: 0.7rem !important;
    ...
    /* Scrollbar */
    * { scrollbar-width: thin; scrollbar-color: var(--panel2) var(--ink); }
    ...
    .SENTI-step.done { color: var(--teal2); }
    .SENTI-step-num {
    ...
    .SENTI-step.done .SENTI-step-num {
        ...
        border-color: rgba(0,196,170,0.35);
        color: var(--teal2);
    }
    .SENTI-step-arrow { color: var(--text3); font-size: 0.6rem; margin: 0 -2px; }
    ...
    .badge-teal { background: var(--teal3); border: 1px solid rgba(0,245,212,0.2); color: var(--teal); }
    ...
    .SENTI-delta-arrow { font-size: 1.3rem; color: var(--text3); }
    .SENTI-delta-change { font-size: 0.78rem; color: var(--teal2); font-weight: 600; }

    /* Pipeline */
    .SENTI-pipeline { display: flex; gap: 0; margin-bottom: 18px; }
    .SENTI-pipeline-step {
        flex: 1; padding: 8px 10px; text-align: center;
        background: rgba(14,16,22,0.5); border: 1px solid var(--rim);
        font-family: var(--mono); font-size: 0.65rem;
    ...
    .SENTI-pipeline-step.done {
        ...
        color: rgba(0,196,170,0.4);
    }

    /* Iter badge */
    .SENTI-iter-badge {
        display: inline-flex; align-items: center; gap: 6px;
        background: var(--teal3); border: 1px solid rgba(0,245,212,0.2);
    ...
    .SENTI-empty-icon { font-size: 2rem; margin-bottom: 12px; opacity: 0.4; }
    .SENTI-empty h4 {
    ...
    .SENTI-empty p {
        font-size: 0.8rem !important; color: var(--text3) !important;
    ...
    .faiss-table th {
        ...
        background: rgba(0,0,0,0.2); border-bottom: 1px solid var(--rim);
    }
    .faiss-table td { padding: 6px 10px; border-bottom: 1px solid rgba(255,255,255,0.03); }
    ...
    .ds-pill-rows { background: var(--sky2); color: var(--sky) !important; border: 1px solid rgba(56,189,248,0.2); }
    .ds-pill-miss { background: var(--amber2); color: var(--amber) !important; border: 1px solid rgba(255,184,0,0.2); }
    .ds-pill-done { background: var(--teal3); color: var(--teal) !important; border: 1px solid rgba(0,245,212,0.2); }
    ...
    .mech-card:hover { border-color: var(--rim2); background: var(--panel); }
    .mech-card.selected {
        border-color: rgba(0,245,212,0.35) !important;
        background: var(--teal3) !important;
    }
    .mech-tag { font-family: var(--mono); font-size: 0.65rem; font-weight: 700; margin-bottom: 5px; }
    .mech-card h4 { font-family: var(--display) !important; font-size: 0.88rem !important; font-weight: 700 !important; color: var(--text) !important; margin-bottom: 3px !important; }
    .mech-card p { font-size: 0.76rem !important; color: var(--text2) !important; line-height: 1.4 !important; margin: 0 !important; }
    ...
    .doc-num {
        ...
        background: var(--teal3); border: 1px solid rgba(0,245,212,0.22);
        display: flex; align-items: center; justify-content: center;
        font-family: var(--display); font-size: 0.82rem;
        font-weight: 700; color: var(--teal);
    }
    .doc-text h4 { font-family: var(--display) !important; font-size: 0.88rem !important; font-weight: 700 !important; color: var(--text) !important; margin-bottom: 4px !important; }
    .doc-text p { font-size: 0.8rem !important; color: var(--text2) !important; line-height: 1.5 !important; margin: 0 !important; }

    /* Column type badges */
    .col-N { display:inline-block; font-family:var(--mono); font-size:0.56rem; padding:1px 4px; border-radius:3px; margin-right:4px; background:var(--sky2); color:var(--sky); border:1px solid rgba(56,189,248,0.2); }
    .col-C { display:inline-block; font-family:var(--mono); font-size:0.56rem; padding:1px 4px; border-radius:3px; margin-right:4px; background:var(--amber2); color:var(--amber); border:1px solid rgba(255,184,0,0.2); }
    ...
    .compare-head-item {
        flex: 1; font-family: var(--mono); font-size: 0.65rem;
        color: var(--text3); text-transform: uppercase; letter-spacing: 0.1em;
        padding-bottom: 4px; border-bottom: 2px solid var(--rim);
    }
    .compare-head-item.imputed { border-color: rgba(0,245,212,0.45); color: var(--teal) !important; }
    .compare-head-item.ground { border-color: rgba(56,189,248,0.45); color: var(--sky) !important; }

    /* Footer */
    .SENTI-footer {
        position: fixed; bottom: 0; left: 0; right: 0; height: 38px;
        display: flex; align-items: center; justify-content: center; gap: 8px;
        background: var(--ink2); border-top: 1px solid var(--rim);
        font-family: var(--mono); font-size: 0.63rem;
        color: var(--text3); z-index: 999;
    }
    .SENTI-footer a { color: var(--teal2); text-decoration: none; }
    ...
    /* Code inline */
    code { background: var(--panel2) !important; border: 1px solid var(--rim) !important; color: var(--teal) !important; font-family: var(--mono) !important; }
...
def _render_topbar(active_page: str = "Imputation"):
    ...
    <div class="SENTI-topbar">
        ...
        <div style="display:flex;flex-direction:column;justify-content:center;gap:1px;">
            <span style="font-family:var(--display);font-size:0.95rem;font-weight:700;
                color:var(--text);letter-spacing:-0.01em;line-height:1.2;">
            </span>
            <span style="font-family:var(--mono);font-size:0.8rem;color:var(--text3);
                letter-spacing:0.1em;text-transform:uppercase;">
                Sentence Transformer based data Imputation
            </span>
        </div>
...
def _render_footer():
    ...
    <div class="SENTI-footer">
        ...
        <a href="https://dimes.unical.it" target="_blank"></a>
        <span class="SENTI-footer-sep"></span>
        <a href="https://dottorato.dimes.unical.it/students/mahmood-tariq" target="_blank"></a>
    </div>
    """, unsafe_allow_html=True)
...
def render_sidebar():
    with st.sidebar:
        ...
        # While docs are open keep radio on the last real page, not "Documentation"
        ...
        if st.button("📄  Documentation", key="btn_nav_docs"):
            real = getattr(ss, "page", "Imputation")
            if real != "Documentation":
                ss._last_real_page = real
            ss._doc = True
            ss.page = "Documentation"
            st.rerun()

        if st.button("↺  Reset Session", key="btn_reset_app_sidebar"):
        ...
        if strategy == "SENTI":
            ...
            if imputed is None:
                ...
                with st.expander("Error log", expanded=True):
                    st.markdown(
                        f'<pre style="background:#0f172a;color:#FCA5A5;font-size:0.74rem;'
                        f'line-height:1.6;padding:14px 16px;border-radius:8px;'
                        f'overflow-x:auto;white-space:pre-wrap;border:1px solid #7f1d1d">'
                        f'{log}</pre>', unsafe_allow_html=True)
                return None, None
            if log.strip():
                with st.expander("Custom imputer log", expanded=False):
                    st.markdown(
                        f'<pre style="background:#0f172a;color:#94A3B8;font-size:0.74rem;'
                        f'line-height:1.6;padding:14px 16px;border-radius:8px;'
...
def render_source_preview(df, iter_k: int):
    ...
    st.markdown(
        f'<div style="font-size:0.72rem;color:var(--text3);font-family:var(--font-mono);margin-top:0.25rem;">'
...
def _render_faiss_panel(latest_only: bool = False):
    ...
    # ── Legend bar ────────────────────────────────────────────────────
    prev_note_faiss = (
        f'<span style="font-size:0.72rem;color:var(--text3);margin-left:8px">'
        f'(+ {n_hidden} previous batch(es) saved to JSON)</span>'
        if n_hidden > 0 else ""
    )
    legend = (
        '<div style="display:flex;gap:18px;flex-wrap:wrap;font-size:0.72rem;'
    ...
    for entry in display_log:
        ...
        batches_html += (
            ...
            # Batch header
            f'<div style="background:rgba(255,255,255,0.04);padding:7px 12px;'
            f'display:flex;gap:20px;align-items:center;border-bottom:1px solid var(--border)">'
            f'<span style="color:var(--accent2);font-weight:700;font-size:0.85rem">Batch #{batch_no}</span>'
            f'<span style="font-size:0.78rem;color:var(--text3)">Context&nbsp;<strong style="color:var(--text2)">{n_ctx}</strong></span>'
            f'<span style="font-size:0.78rem;color:var(--accent3)">+New&nbsp;<strong>{n_new}</strong></span>'
            f'<span style="font-size:0.78rem;color:var(--green)">Total&nbsp;<strong>{n_after}</strong></span>'
            f'<span style="font-size:0.78rem;color:{sum_color};font-weight:700">{sum_icon} Δ={n_after - n_ctx}</span>'
            f'<span style="font-size:0.72rem;color:var(--text3);margin-left:auto">dim={dim}</span>'
            f'</div>'
            # Phase sub-table
            f'<table style="width:100%;border-collapse:collapse;font-size:0.78rem">'
            f'<thead><tr style="color:var(--text3);font-size:0.67rem;letter-spacing:0.07em;'
            f'border-bottom:1px solid var(--border);background:rgba(0,0,0,0.15)">'
        ...
        for ph in phases:
            ...
            batches_html += (
                ...
                f'<td style="padding:5px 12px;text-align:right;color:var(--text3);font-size:0.72rem">{ph_norm}</td>'
                f'</tr>'
            )
    ...
    footer_html = (
        f'<p style="font-size:0.74rem;color:var(--text3);margin:10px 0 0;line-height:1.7;'
...
def show_imputed(imputed, mask, title="Imputed Dataset"):
    ...
    st.markdown(f"""
    <div style="margin-top:0.3rem; font-size:0.72rem; color:var(--text3); font-family:var(--font-mono);">
    ...
    with hint_col:
        st.markdown(
            f'<div style="padding-top:0.55rem; font-size:0.72rem; color:var(--text3);">'
            f'Saved as <code style="color:var(--accent3); font-size:0.7rem;">imputed_{_ts}.csv</code></div>',
            unsafe_allow_html=True
        )
...
def data_loader_ui():
   # _section("Dataset", badge="INPUT")
    ...
    with left:
        ...
        if ss.demo_toggle:
            _demo_opts = ["Demographic", "Health", "Cargo"]
...
def page_null_injection():
    ...
    <div class="SENTI-card">
        <div class="SENTI-card-label">About this step</div>
        <h4>Controlled Missingness Injection</h4>
        <p>Upload a complete dataset and introduce missing values to simulate real-world incomplete data.
        Choose between two injection engines:</p>
        <ul style="margin:0.4rem 0 0 1rem; line-height:1.9">
            ...
            that lets you define multiple simultaneous missing-data patterns, each with its own
            mechanism, frequency, and weighted-score function — exactly as used in academic
...
def _page_null_builtin():
  ...
  #  </div>
   #""", unsafe_allow_html=True)
    ...
    if uploaded is None:
        st.markdown('<p style="color:var(--text3); font-size:0.85rem;">No file selected yet.</p>', unsafe_allow_html=True)
        return
    ...
    if mech == "MAR":
        ...
        for i, cond in enumerate(st.session_state["mar_conditions"]):
            if i > 0:
                st.markdown(f'<div style="text-align:center;font-size:0.72rem;font-weight:700;color:var(--accent2);margin:2px 0;">{mar_combiner}</div>', unsafe_allow_html=True)
...
def _page_null_pyampute():
  ...
  #  st.markdown("""
   # <div class="SENTI-card" style="padding:10px 16px; margin-bottom:0.8rem;">
    #    <strong>pyampute</strong> implements <em>multivariate amputation</em>:
    #    you define <strong>patterns</strong>, each specifying which columns go missing,
     #   how often, under which mechanism (MCAR / MAR / MNAR) and — for MAR/MNAR —
     #   which other columns influence the probability of a row being selected.
     #   <br><br>
      #  <a href="https://github.com/RianneSchouten/pyampute" target="_blank">
     #   📦 github.com/RianneSchouten/pyampute</a>
       # &nbsp;·&nbsp;
      #  <a href="https://rianneschouten.github.io/pyampute/build/html/index.html" target="_blank">
     #   📖 Documentation</a>
   # </div>
  #  """, unsafe_allow_html=True)
    ...
    for pi, pat in enumerate(patterns_state):
        ...
        st.markdown(
            ...
            f'margin:0.6rem 0 0.2rem 0; font-weight:700; font-size:0.9rem;">'
            f'Pattern {pi + 1}</div>',
            unsafe_allow_html=True
        )
...
def _custom_imputer_panel():
    ...
    .repo-card-name { font-size:0.82rem; font-weight:700; color:#E2E8F0 !important; margin-bottom:3px; }
    .repo-card-url  { font-size:0.72rem; color:#60A5FA !important; font-family:monospace;
                      word-break:break-all; margin-bottom:3px; }
    .repo-card-desc { font-size:0.74rem; color:#94A3B8 !important; line-height:1.5; }
    </style>
    """, unsafe_allow_html=True)
    ...
    if info and repo_path:
        ...
        with st.container(border=True):
            ...
            if info.get("functions"):
                ...
                for fn in info["functions"]:
                    ...
                    fn_html += (
                        ...
                        f'<span style="font-size:0.70rem;color:#64748B;margin-right:10px"> in {fn["file"]}</span>'
                    )
            ...
            if info.get("classes"):
                ...
                for cls in info["classes"]:
                    ...
                    cls_html += (
                        ...
                        f'<span style="font-size:0.70rem;color:#64748B;margin-right:10px"> {methods_str} · {cls["file"]}</span>'
                    )
            ...
            if reqs:
                ...
                if len(reqs) > 14:
                    req_html += f'<span style="font-size:0.72rem;color:#64748B"> +{len(reqs)-14} more</span>'
            ...
            if snippet:
                with st.expander("README preview", expanded=False):
                    st.markdown(
                        f'<div style="font-size:0.80rem;color:#CBD5E1;line-height:1.7;white-space:pre-wrap">{snippet[:600]}</div>',
                        unsafe_allow_html=True)
        ...
        with st.container(border=True):
            ...
            if log:
                if ok:
                    ...
                    if error_lines:
                        summary_html = (
                            '<div style="background:#1a0f0f;border:1px solid #7f1d1d;border-radius:8px;'
                        ...
                        for el in error_lines[:10]:
                            summary_html += (
                                ...
                                f'color:#FCA5A5;padding:2px 0;line-height:1.6">{el}</div>'
                            )
                        if len(error_lines) > 10:
                            summary_html += (
                                f'<div style="font-size:0.73rem;color:#64748B;margin-top:4px">'
                    ...
                    if matched:
                        for title, explanation, fix_cmds in matched:
                            diag_html = (
                                ...
                                + title +
                                '</div>'
                                '<div style="font-size:0.80rem;color:#CBD5E1;line-height:1.7;margin-bottom:10px">'
                                + explanation +
                                '</div>'
                                '<div style="font-size:0.74rem;font-weight:600;color:#94A3B8;'
                                'margin-bottom:4px">Suggested fix:</div>'
                                '<pre style="background:#0a0a1a;color:#86EFAC;font-size:0.74rem;'
...
def page_senti():
    ...
    with st.expander("📖  SENTI Toy Example (click to expand)", expanded=False):
        ...
        def _toy_style_imputed(df_s, mask_df):
            ...
            for r in df_s.index:
                for c in df_s.columns:
                    if mask_df.at[r, c]:
                        styles.at[r, c] = "background-color:rgba(0,245,212,0.15);color:#00F5D4;font-weight:700"
            return styles
        ...
        # ── r0 / r0* ─────────────────────────────────────────────────
        st.markdown('<div class="SENTI-card-label" style="margin-bottom:6px;">BATCH r₀ — Initial Relation</div>', unsafe_allow_html=True)
        r0_inc = pd.DataFrame([
        ...
        cl, cr = st.columns(2)
        with cl:
            st.markdown('<div class="SENTI-card-label">r₀ — Incomplete (⊥ = missing)</div>', unsafe_allow_html=True)
            st.dataframe(
        ...
        with cl2:
            st.markdown('<div class="SENTI-card-label">u₁ — Incomplete</div>', unsafe_allow_html=True)
            st.dataframe(
    ...
    # ── Mode / Strategy / Transformer selector ────────────────────
   # _section("Configuration", badge="SETUP")
    m1, m2, m3 = st.columns(3)
    with m1:
        st.markdown('<div class="SENTI-card-label">Mode</div>', unsafe_allow_html=True)
        ss.mode = st.selectbox(
            "Mode",
            options=["Static Imputation", "Incremental Imputation"],
            index=(["Static Imputation", "Incremental Imputation"].index(ss.mode)
                   if ss.mode in ["Static Imputation", "Incremental Imputation"] else 0),
    ...
    with m2:
        ...
        strategy_label = st.selectbox(
            "Strategy",
            ["SENTI (semantic)", "Mean", "Median", "Mode", "KNN (k=5)", "Forward-fill", "🔌 Custom (GitHub)"],
            key="SENTI_strategy_sel", label_visibility="collapsed",
            index=0,
        )
        _strat_map = {
            "SENTI (semantic)": "SENTI", "Mean": "mean", "Median": "median",
            "Mode": "mode", "KNN (k=5)": "knn", "Forward-fill": "ffill",
            "🔌 Custom (GitHub)": "custom",
        }
        ss.strategy = _strat_map.get(strategy_label, "SENTI")
        ss.other_strategy = ss.strategy
    with m3:
        if ss.strategy == "SENTI":
            st.markdown('<div class="SENTI-card-label">Sentence Transformer</div>', unsafe_allow_html=True)
        ...
        elif ss.strategy == "custom":
            st.markdown('<div class="SENTI-card-label">GitHub repo</div>', unsafe_allow_html=True)
            st.markdown(
                '<div style="font-size:0.78rem;color:var(--text2);line-height:1.55;padding-top:3px">'
                'Paste any imputation repo URL → SENTI clones it, inspects it, '
                'generates an adapter, and runs it in an isolated subprocess.'
                '</div>', unsafe_allow_html=True)
        else:
            _desc = {
                "mean":  "Replaces nulls with column mean (numeric).",
                "median":"Replaces nulls with column median (numeric).",
                "mode":  "Replaces nulls with most frequent value.",
                "knn":   "k-NN imputer (k=5) on numeric cols.",
                "ffill": "Forward-fill then backward-fill by row order.",
            }
            st.markdown('<div class="SENTI-card-label">Baseline Info</div>', unsafe_allow_html=True)
            st.markdown(f'<div style="font-size:0.80rem; color:var(--text2); padding-top:0.4rem; line-height:1.6;">{_desc.get(ss.strategy, "")}</div>',
                        unsafe_allow_html=True)
    ...
    if ss.working_df is not None:
        ...
        st.markdown(f"""
        <div style="display:flex; gap:10px; margin-bottom:0.6rem; flex-wrap:wrap; font-size:0.75rem; color:var(--text2);">
        ...
        if ss.get("_col_sel_df_sig") != _df_sig:
            # Dataset changed — reset everything
            ss._col_sel_df_sig  = _df_sig
            ss._impute_col_mode = "all"
            ss.selected_cols    = missing_cols[:]

        if not getattr(ss, "_impute_col_mode", None):
            ss._impute_col_mode = "all"

        # Quick-select buttons — set state directly, no rerun needed
        col_q1, col_q2, col_q3, col_q4, _ = st.columns([1, 1.3, 1.3, 1.3, 1.5])
        with col_q1:
            if st.button("Select all", key="btn_quick_all"):
                ss._impute_col_mode = "all"
                ss.selected_cols    = missing_cols[:]
        with col_q2:
            if st.button("Categorical only", key="btn_quick_cat"):
                cats = [c for c in missing_cols
                        if not pd.api.types.is_numeric_dtype(ss.working_df[c])]
                ss._impute_col_mode = "cat"
                ss.selected_cols    = cats
        with col_q3:
            if st.button("Numerical only", key="btn_quick_num"):
                nums = [c for c in missing_cols
                        if pd.api.types.is_numeric_dtype(ss.working_df[c])]
                ss._impute_col_mode = "num"
                ss.selected_cols    = nums
        with col_q4:
            if st.button("Manual select", key="btn_quick_manual"):
                ss._impute_col_mode = "manual"

        # Always keep selected_cols valid
        valid_stored = [c for c in (ss.selected_cols or []) if c in missing_cols]
        if not valid_stored:
            valid_stored    = missing_cols[:]
            ss.selected_cols = valid_stored

        if ss._impute_col_mode == "manual":
            # Multiselect drives selection directly — no default= so it doesn't override state
            if "cols_to_impute_multiselect" not in st.session_state or                not isinstance(st.session_state["cols_to_impute_multiselect"], list):
                st.session_state["cols_to_impute_multiselect"] = valid_stored
            ...
            # Non-manual: show active selection as read-only chips
            sel_label = ", ".join(valid_stored) if valid_stored else "none"
            st.markdown(
                f'<div style="font-size:0.78rem;color:var(--text2);padding:4px 0;">' 
    ...
    if ss.working_df is not None:
        if ss.mode == "Static Imputation":
            render_source_preview(ss.working_df, 0)
            st.markdown('<div style="margin-top:1rem"></div>', unsafe_allow_html=True)
            run_col, hint_col = st.columns([2, 6])
            with run_col:
                do_run = st.button("▶  Run Imputation", type="primary", key="btn_run_impute_static")
            with hint_col:
                st.markdown(
                    '<div style="padding-top:0.5rem; font-size:0.72rem; color:var(--text3);">'
                    f'Imputer: <code style="color:var(--accent3);">{ss.strategy}</code>'
                    + (f' · <code style="color:var(--accent3);">{ss.transformer_choice}</code>' if ss.strategy == "SENTI" else "")
                    + '</div>',
                    unsafe_allow_html=True,
                )
            if do_run:
                ...
                if imputed is not None:
                    ss.imputed_df = imputed
                    ss.imputed_mask = mask
            # Persistent display: show on every render once result exists
            if ss.imputed_df is not None:
                show_imputed(ss.imputed_df, ss.imputed_mask)
                _render_faiss_panel()

        else:
            # ── Incremental mode ────────────────────────────────────
            _section("Incremental Imputation", badge="ITERATIVE")

            # Pipeline progress
            bc = ss.iter_k or 1
            step_labels = [("01", "LOAD DATASET"), ("02", "RUN IMPUTATION"), ("03", "APPEND TUPLES"), ("04", "EXPORT")]
            steps_html = ""
            for i, (num, label) in enumerate(step_labels):
                done = (bc > i + 1)
                active = (bc == i + 1) or (i == 0 and bc == 1)
                cls = "done" if done else ("active" if active else "")
                steps_html += f'<div class="SENTI-pipeline-step {cls}">{num} {label}</div>'
            st.markdown(f'<div class="SENTI-pipeline">{steps_html}</div>', unsafe_allow_html=True)
            ...
            if st.button("▶  Run Imputation", type="primary", key=f"btn_run_impute_dyn_initial_{ss.iter_k}"):
                ss.source_snapshot = ss.working_df.copy()
                with st.spinner("Running imputation…"):
                    imputed, mask = run_imputation(ss.working_df, context_df=None)
                if imputed is not None:
            ...
            if ss.imputed_df is not None and ss.flow_state in ("post_impute_prompt", "append_phase") and ss.last_imputed_iter == ss.iter_k:
                ...
                if inc_log:
                    ...
                    last_row_html = (
                        ...
                        f'<td style="color:var(--text3);font-size:0.75rem">{last_entry["method"]}</td>'
                        f'</tr>'
                    )
                    st.markdown(f"""
                    <div class="SENTI-card" style="margin-top:1rem">
                      ...
                      <div style="overflow-x:auto"><table style="width:100%;border-collapse:collapse;font-size:0.82rem">
                        <thead><tr style="color:var(--text3);font-size:0.70rem;letter-spacing:0.08em">
                        ...
                        if num_drift_cols:
                            ...
                            for col in num_drift_cols[:6]:
                                ...
                                drift_items += (
                                    ...
                                    f'<span style="font-size:0.82rem;color:var(--text)">{col}</span>'
                                    f'<span style="font-size:0.82rem;color:{d_color};font-weight:700">'
                                    f'{arrow} {drift_pct:.1f}% drift</span>'
                                    f'<span style="font-size:0.75rem;color:var(--text3)">'
                            ...
                            if drift_items:
                                ...
                                <div class="SENTI-card" style="margin-top:0.8rem">
                                  ...
                                  <p style="font-size:0.78rem;color:var(--text2);margin:0 0 8px;line-height:1.5">
...
def page_docs():
    ...
    _section("Documentation", badge="GUIDE")
    ...
    .doc-h3 {
        font-size: 0.90rem; font-weight: 600; color: var(--accent2);
        margin: 1.1rem 0 0.3rem;
    }
    .doc-p {
        font-size: 0.83rem; color: var(--text2); line-height: 1.75;
    ...
    .doc-note {
        ...
        font-size: 0.80rem; color: var(--text2); line-height: 1.6;
    }
    .doc-warn {
    ...
    .doc-step-body h4 { font-size: 0.87rem; color: var(--text); margin: 0 0 3px; }
    .doc-step-body p  { font-size: 0.80rem; color: var(--text2); margin: 0; line-height: 1.6; }
    .doc-phase {
        display: flex; gap: 12px; margin: 8px 0; align-items: flex-start;
    }
    .doc-phase-badge {
        min-width: 72px; padding: 3px 8px; border-radius: 12px;
        font-size: 0.70rem; font-weight: 700; text-align: center;
        flex-shrink: 0; margin-top: 2px;
    }
    .doc-phase-text { font-size: 0.80rem; color: var(--text2); line-height: 1.65; }
    .doc-kv { display: flex; gap: 10px; margin: 4px 0; font-size: 0.80rem; }
    ...
    .doc-metric-chip {
        padding: 5px 14px; border-radius: 20px; font-size: 0.77rem;
        font-weight: 600; background: rgba(96,165,250,0.12);
        color: var(--accent2); border: 1px solid rgba(96,165,250,0.25);
    }
    .doc-fk-arrow {
        display: inline-flex; align-items: center; gap: 6px;
        font-size: 0.80rem; padding: 4px 10px; border-radius: 16px;
    ...
    with tabs[0]:
        st.markdown("""
        <div class="doc-p" style="font-size:0.88rem">
        ...
        pages = [
            ...
            ("Documentation",  "var(--text3)",   "This page."),
        ]
...
def page_eval():
    _section("Evaluation", badge="METRICS")

    st.markdown("""
    <div class="SENTI-card">
        <div class="SENTI-card-label">Instructions</div>
...
def main():
    ...
    if getattr(ss, "_doc", False) or str(page) == "Documentation":
        page_docs()

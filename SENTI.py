...
def _coerce_to_nullable_dtypes(df: pd.DataFrame) -> pd.DataFrame:
    """
    Convert a DataFrame's columns to pandas nullable dtypes so that missing
    values do NOT silently downcast integers to float64.

    Key fix: when pandas reads a CSV with NaNs in an integer column it
    silently produces float64 (e.g. age → 19.0, 26.0, NaN).
    We detect those "disguised integer" float columns — where every non-null
    value is a whole number — and cast them back to Int64.

    Mapping applied per column:
      already int8/16/32/64            → Int8/16/32/64  (nullable)
      float column where all non-null
        values are whole numbers       → Int64           (recover lost int type)
      other float32/64                 → Float32/64      (nullable)
    ...
    for col in out.columns:
        s = out[col]
        kind = s.dtype.kind   # 'i'=int 'u'=uint 'f'=float 'b'=bool 'O'=object

        if kind in ("i", "u"):
            # Already integer — just make it nullable
            bits = min(getattr(s.dtype, "itemsize", 8) * 8, 64)
            try:
                out[col] = s.astype(f"Int{bits}")
            except (TypeError, ValueError):
                pass

        elif kind == "f":
            # Check if this float column is actually a disguised integer column
            # (all non-null values are whole numbers, e.g. 19.0, 26.0, NaN)
            non_null = s.dropna()
            if len(non_null) > 0 and np.all(non_null == non_null.astype("int64")):
                # It's a disguised integer — recover as Int64
                try:
                    out[col] = s.astype("Int64")
                    continue
                except (TypeError, ValueError, OverflowError):
                    pass
            # Genuine float — use nullable Float64
            bits = 32 if getattr(s.dtype, "itemsize", 8) * 8 <= 32 else 64
            try:
                out[col] = s.astype(f"Float{bits}")
            except (TypeError, ValueError):
                pass
            ...
            try:
                ...
                pass
        # object/string columns: leave as-is

    return out


def _align_dtypes_to_reference(df: pd.DataFrame, ref: pd.DataFrame) -> pd.DataFrame:
    """
    Cast each column of *df* to match the dtype of the same column in *ref*,
    where possible.  Used when appending an uploaded incomplete file so its
    columns match the working_df's dtypes.
    """
    out = df.copy()
    for col in out.columns:
        if col not in ref.columns:
            continue
        target_dtype = ref[col].dtype
        if out[col].dtype == target_dtype:
            continue
        try:
            out[col] = out[col].astype(target_dtype)
        except (TypeError, ValueError):
            # Best-effort; leave unchanged if cast fails
            pass
    return out
...
def _read_uploaded_file(uploaded_file) -> pd.DataFrame:
    ...
    Numeric columns with missing values are read with pandas nullable dtypes
    (Int8/16/32/64, Float32/64, boolean) so that integer columns do NOT get
    silently downcast to float64 just because NaNs are present.
    """
    ...
    try:
        if ext in ("csv", "tsv", ""):
            ...
            # Parquet already encodes dtypes precisely; skip coercion
    ...
    # Convert to pandas nullable dtypes to preserve integer/boolean columns
    # even when NaNs are present (avoids the float64 downcast problem).
    return _coerce_to_nullable_dtypes(df)
    ...
    try:
        ...
        if strategy == "SENTI":
            imputed, mask, faiss_stats = impute_senti(
                df, cols,
                getattr(ss, "transformer_choice", "all-MiniLM-L6-v2"),
                ...
                try:
                    ...
                    if num_cols_knn:
                        ...
                        for i, c in enumerate(num_cols_knn):
                            imputed[c] = filled[:, i]
                        # Restore original dtypes (KNN returns float64 for all numeric)
                        for c in num_cols_knn:
                            try:
    ...
    with c_csv:
        ...
        if up is not None:
            ...
            if ss.get("_last_appended_file_id") != _file_id:
                ...
                df_new = _align_dtypes_to_reference(df_new, ss.working_df)
...
def page_senti():
    ...
    if _current_strat == "SENTI":
        ...
        with _tc1:
            ...
            ss.transformer_choice = st.selectbox(
                "Transformer",
                options=["all-MiniLM-L6-v2", "LaBSE"],
                key="transformer_choice_inpanel",
                index=(["all-MiniLM-L6-v2", "LaBSE"].index(ss.get("transformer_choice", "all-MiniLM-L6-v2"))
                       if ss.get("transformer_choice") in ["all-MiniLM-L6-v2", "LaBSE"] else 0),
                label_visibility="collapsed",
            )
        with _tc2:
            _model_info = {
                "all-MiniLM-L6-v2": ("", "var(--green, #16a34a)"),
                "LaBSE":            ("", "#b45309"),
            }
            _info_text, _info_color = _model_info.get(ss.get("transformer_choice", "all-MiniLM-L6-v2"), ("", ""))
            st.markdown(
                f'<div style="font-size:1.05rem;color:{_info_color};margin-top:2rem;line-height:1.4;">' +
                f'{_info_text}</div>',
                unsafe_allow_html=True,
            )
    ...
    if ss.working_df is not None:
        ...
        # Column-mode radio — four options
        _mode_labels = ["Select all", "Categorical only", "Numerical only", "Manual"]
        _mode_keys   = ["all", "cat", "num", "manual"]
        ...
        _num_missing = [c for c in missing_cols if pd.api.types.is_numeric_dtype(ss.working_df[c])]
        _cat_missing = [c for c in missing_cols if not pd.api.types.is_numeric_dtype(ss.working_df[c])]

        if _sel_key == "all":
            ss.selected_cols = missing_cols[:]
        elif _sel_key == "cat":
            ss.selected_cols = _cat_missing[:]
        elif _sel_key == "num":
            ss.selected_cols = _num_missing[:]
    ...
    if ss.working_df is not None:
        ...
        if _do_run:
            ss.source_snapshot = ss.working_df.copy()
            _model_used = ss.get("transformer_choice", "all-MiniLM-L6-v2")
            _spinner_msg = (
                f"Running imputation with {_model_used}… "
                + ("(downloading ~471 MB model on first run — this may take a few minutes)" if _model_used == "LaBSE" else
                   "(downloading ~22 MB model on first run…)" if not ss.get("_model_cached_" + _model_used) else "")
            )
            with st.spinner(_spinner_msg):
                imputed, mask = run_imputation(ss.working_df, context_df=None)
            ss["_model_cached_" + _model_used] = True
            if imputed is not None:
                ss.last_imputed_iter = ss.iter_k
                ss.imputed_df = imputed
                ss.imputed_mask = mask

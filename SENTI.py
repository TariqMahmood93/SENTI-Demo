...
def page_senti():
    ...
    if ss.working_df is not None:
        ...
        # Derive missing columns by type
        missing_num_cols = [c for c in missing_cols if pd.api.types.is_numeric_dtype(ss.working_df[c])]
        missing_cat_cols = [c for c in missing_cols if not pd.api.types.is_numeric_dtype(ss.working_df[c])]
        ...
        if ss.get("_col_sel_df_sig") != _df_sig:
            ...
            # Reset radio to "Select all" when dataset changes
        ...
        _mode_keys   = ["all", "categorical", "numerical", "manual"]
        ...
        if _sel_key == "all":
            ss.selected_cols = missing_cols[:]
        elif _sel_key == "categorical":
            ss.selected_cols = missing_cat_cols[:]
        elif _sel_key == "numerical":
            ss.selected_cols = missing_num_cols[:]

        if _sel_key == "manual":
            _manual_default = [c for c in (ss.selected_cols or []) if c in missing_cols] or missing_cols[:]
            # Always validate the stored value against the current missing_cols.
            # If the key is missing, wrong type, or contains columns not in missing_cols, reset it.
            _stored = st.session_state.get("cols_to_impute_multiselect")
            if not isinstance(_stored, list) or not all(c in missing_cols for c in _stored):
